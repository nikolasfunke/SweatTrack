...
import MedidorHidratacao from '../components/charts/MedidorHidratacao';
import GraficoSemanal from '../components/charts/GraficoSemanal';
import CartaoSessao from '../components/session/CartaoSessao';
import Modal from '../components/ui/Modal';
import { useToast } from '../components/ui/Toast';
import { getSweatRateLabel } from '../utils/calculos';
    ...
    <LayoutApp>
      ...
      <div className="page-container md:max-w-4xl">
        <motion.div variants={stagger} initial="initial" animate="animate" className="space-y-5">
          ...
          {/* Hydration card */}
          <motion.div variants={fadeUp}>
            <Cartao glow className="relative overflow-hidden">
              ...
              <div className="flex items-center gap-6 mt-3">
                {hydrationIndex !== null ? (
                  <MedidorHidratacao value={hydrationIndex} size={140} />
                ) : (
          ...
          <motion.div variants={fadeUp}>
            <Cartao>
              ...
              {isVirgin ? (
                ...
                <GraficoSemanal weeklyData={analytics?.weeklyData} />
              )}
            </Cartao>
          </motion.div>
              ...
              {sessions.length === 0 ? (
                ...
                sessions.slice(0, 5).map((s) => (
                  <CartaoSessao
                    ...
                    onClick={() =>
                      s.status === 'completed'
                        ? navigate(`/pos-sessao/${s.id}`)
                        : navigate(`/pre-sessao/${s.id}`)
                    }
                  />
                ))
              )}
      ...
      <Modal open={showNewSession} onClose={() => setShowNewSession(false)} title="Nova Sessão">
        <div className="space-y-4">
          ...
          <Botao variant="primary" size="xl" loading={creating} onClick={handleCreateSession}>
            Criar Sessão
          </Botao>
        </div>
      </Modal>
...
function EmptyState({ onNew }) {
  return (
    <div className="flex flex-col items-center justify-center py-10 gap-4 text-center">
      ...
      <Botao variant="primary" size="sm" onClick={onNew} icon={<Plus size={14} />}>
