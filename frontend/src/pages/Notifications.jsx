...
import { Bell, Droplets, Activity, AlertTriangle, Salad, CheckCheck, ShieldCheck, ShieldX, Shield, Users } from 'lucide-react';
import { userApi, adminApi, teamApi } from '../services/api';
...
const TYPE_ICON = {
  ...
  team_invite:   <Users size={15} className="text-primary" />,
  team_request:  <Users size={15} className="text-primary" />,
};
...
function TeamActionCard({ n, onAction }) {
  const [loading, setLoading] = useState(null);
...
export default function Notifications() {
  ...
  const handleTeamAction = async (notificationId, action) => {
    try {
      await teamApi.respondFromNotification(notificationId, action);
      setNotifications((prev) =>
        prev.map((x) => x.id === notificationId ? { ...x, action_taken: 1, is_read: 1 } : x)
      );
      toast(action === 'accept' ? 'Solicitação aceita com sucesso!' : 'Solicitação recusada', 'success');
    } catch {
      toast('Erro ao processar ação de equipe', 'error');
    }
  };
        ...
        {loaded && notifications.length === 0 ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-1">
            ...
            {notifications.map((n, i) => {
              ...
              if (n.type === 'team_invite' || n.type === 'team_request') {
                return (
                  <div key={n.id}>
                    {isFirstRead && <p className="section-title px-1 mt-4 mb-2">Anteriores</p>}
                    <TeamActionCard n={n} onAction={handleTeamAction} />
                  </div>
                );
              }
