  ...
  return (
    <LayoutApp>
      <Cabecalho
        title="Monitorar"
        actions={
          <Botao variant="primary" size="sm" icon={<Plus size={14} />} onClick={() => setShowModal(true)}>
            Nova Sessão
          </Botao>
        }
      />
            ...
            <div className="bg-gradient-to-br from-primary/20 to-rose-900/10 border border-primary/20 rounded-2xl p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <Botao variant="primary" size="sm" icon={<Zap size={14} />} onClick={() => setShowModal(true)}>
                    Nova Sessão
                  </Botao>
                </div>
              </div>
              ...
              <div className="space-y-3">
                {recentDone.map((s) => (
                  <CartaoSessao
                    key={s.id}
                    session={s}
                    onClick={() => navigate(`/pos-sessao/${s.id}`)}
                  />
                ))}
              </div>
            </motion.div>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Nova Sessão">
        <div className="space-y-4">
          ...
          <Botao variant="primary" size="xl" loading={creating} onClick={handleCreate}>
            Criar Sessão
          </Botao>
        </div>
      </Modal>
