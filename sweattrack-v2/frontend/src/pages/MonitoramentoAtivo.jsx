...
import { formatTimer } from '../utils/calculos';

const FLUID_OPTIONS = [150, 250, 500];

export default function MonitoramentoAtivo() {
  const { id } = useParams();
  ...
  const handleUpdateTemp = async () => {
    ...
    try {
      ...
      setNewTemp('');
      toast('Temperatura actualizada', 'success');
    } catch {
  ...
  return (
    <LayoutApp>
      <Cabecalho title="Monitoramento Ativo" showBack />
      ...
      <Modal open={showTempModal} onClose={() => setShowTempModal(false)} title="Atualizar Temperatura">
        <div className="space-y-4">
          <Input
            ...
            suffix="°C"
          />
          <Botao variant="primary" size="xl" onClick={handleUpdateTemp}>
            Atualizar
          </Botao>
        </div>
      </Modal>
      ...
      <Modal open={showFinish} onClose={() => setShowFinish(false)} title="Encerrar Sessão">
        <div className="space-y-4">
          ...
          <Botao variant="danger" size="xl" loading={finishing} onClick={handleFinish}
